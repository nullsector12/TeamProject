...
//공격을 정의한다.
	...
	void monsterAttack(Player p, int dmg) {
		...
		p.setCurrentHealth(p.getCurrentHealth()-dmg);
		System.out.println(dmg + " 만큼 가격!" + p.getName() + " 플레이어님의 체력은:" +p.getCurrentHealth());
	...
	void choicePlayerMovement(Monster m, Player p) {
		...
		while (true) {
			...
			if (p.getCurrentHealth() <= 0) {
				...
				m.setGoldWorth(m.getExpWorth());
				System.out.println(m.getName()+" 경험치: "+m.getExpWorth()+" 골드: "+m.getGoldWorth());
			}
		}
	}
}